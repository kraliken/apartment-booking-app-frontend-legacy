...
import AdminTableCellActive from './AdminTableCellActive'
...
const AdminCalendar = ({ 
  ...
  return (
    <>
            ...
            return (
              ...
              // ha van ilyen checkin és egyezik a hónap, akkor piros
                ...
                ? <TableCell
                    ...
                    colvalue={col.value}
                  />
                // ha van ilyen admin checkin és nem egyezik a hónap, akkor inactive és narancs
                : adminDates && adminDates.includes(col.date) && new Date(col.date).getMonth() !== selectedDate.getMonth()
                ? <TableCell
                    key={`${col.date}-checkin`}
                    colclass={`${col.classes} inactive custom-orange border border-white p-0 col-1`}
                    colvalue={col.value}
                  />
                // ha van ilyen admin checkin és egyezik a hónap, akkor narancs
                : adminDates && adminDates.includes(col.date) && new Date(col.date).getMonth() === selectedDate.getMonth()
                ? <TableCell
                    key={`${col.date}-checkin`}
                    colclass={`${col.classes} admin custom-orange border border-white p-0 col-1`}
                    colvalue={col.value}
                  />
                : new Date(col.date).getMonth() === selectedDate.getMonth() && today.toLocaleString().substr(0, 13) < col.date
                ? <AdminTableCellActive
                    key={`${col.date}-checkin`}
                    coldate={col.date}
                    darkgreen={`${col.classes} cursor selected text-center border border-white p-0 col-1`}
                    colclassgreen={`${col.classes} cursor custom-green text-center border border-white p-0 col-1`}
                    colvalue={col.value}
                    setNewAdminReservations={setNewAdminReservations}
                    newAdminReservations={newAdminReservations}
                  />
                : adminReservations.datesArray && adminReservations.datesArray.includes(col.date) && new Date(col.date).getMonth() !== selectedDate.getMonth()
                ? <TableCell
                    key={`${col.date}-checkin`}
                    colclass={`${col.classes} inactive custom-red border border-white p-0 col-1`}
                    colvalue={col.value}
      ...
      <Row className="admin-calendar-text">
        <Col>
          <Badge className="custom-red fw-normal mb-1">vendég foglalás</Badge >
        </Col>
      </Row>                 
      <Row className="admin-calendar-text">
        <Col>
          <Badge className="custom-orange fw-normal mb-1">admin foglalás</Badge >
        </Col>
      </Row>                 
    </>
  )
